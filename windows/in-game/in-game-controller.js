...
import { kHotkeyToggle, kHotkeyToggleHeader } from '../../scripts/constants/hotkeys-ids.js';
...
export class InGameController {
  constructor() {
    ...
    this._windowOpacity = localStorage.getItem('ingameWindowOpacity');
    this._chatOpacity = localStorage.getItem('ingameChatOpacity');
    this._headerVisible = localStorage.getItem('ingameHeaderShow');
    this._embedURL = localStorage.getItem('ingameEmbedURL');
  ...
  run() {
    ...
    this.hotkeysService.setToggleHotkeyListener(kHotkeyToggleHeader, () => {
      this._onHotkeyToggleHeaderPressed();
    });
  ...
  async _updateHotkey() {
    ...
    this.inGameView.updateToggleHeaderHotkey(kHotkeyToggleHeader);
  }

  
  _onHotkeyToggleHeaderPressed() {
      $(".app-header").toggle();
      localStorage.setItem('ingameHeaderShow', $(".app-header").is(":visible"));
      return;
  }
